...
var y;
var cname;
var CountryFlags = {"Afghanistan": {"mini": "http://flags.fmcdn.net/data/flags/mini/af.png", "normal": "http://flags.fmcdn.net/data/flags/normal/af.png"}, 
...
document.addEventListener('DOMContentLoaded', function () {
 ...
 d3.select("#sub").on("click", function() {
  submit();
    ...
    svg = d3.select("#my_dataviz1").attr("width", 800).attr("height", 600).append("g").attr("transform","translate(" + 40+ "," + 40 + ")");
    ...
    yAxis = svg.append("g").attr("class", "myYaxis") 
     ...
     Promise.all([d3.csv('data/global_development.csv'),d3.csv('data/countries_regions.csv')])
        .then(function (values) {
          ...
          a.addEventListener("change",submit);
          var a1=document.getElementById("check2");
          a1.addEventListener("change",submit);
          var a2=document.getElementById("check3");
          a2.addEventListener("change",submit);
          var a3=document.getElementById("check4");
          a3.addEventListener("change",submit);
          var a4=document.getElementById("check5");
          a4.addEventListener("change",submit);
          var a5=document.getElementById("check6");
          a5.addEventListener("change",submit);
          var a6=document.getElementById("check7");
          a6.addEventListener("change",submit);
          var a7=document.getElementById("Attributes");
          a7.addEventListener("change",submit);
...
data1=mergeById(arr1, arr2);

        for (const key in values[0][0]) {
    
      ans_set.push(key);
      }
      d3.select("#Attributes").selectAll('options').data(ans_set.slice(2,-1)).enter().append('option').text(d=>{return d;}).attr("attributes",function(d){return d;});
      
      for (var x of values[1]) { 
        
        if(x["World bank region"] in hash)
        {
          let a=hash[x["World bank region"]];
          a.push(x["Country"]);
          hash[x["World bank region"]]=a;
        } 
        else
        {
          let a=[];
    ...
    startButton.on('click', toggleInterval);
...
function myTimer() {
  ...
  const max = +rangeSlider.property("max");
  const value = +rangeSlider.property("value");
  const t = (value + 1) % (max + 1) || +rangeSlider.property("min");
  rangeSlider.property("value", t);
  submit();
}

function eventCheckBox() {
  const checkboxIds = ["check1", "check2", "check3", "check4", "check5", "check6", "check7"];
  const checkAll = document.getElementById("check0");
  ...
  function submit()
      {
          var ans=Array();
          var d1=[];
          cname=document.getElementById('name').checked;
          var year0=document.getElementById('year0').value;
          var year1=document.getElementById('year1').value;
          var opacity=document.getElementById('opacity').value;
          var slider=document.getElementById('rangeSlider').value;
          var a=document.getElementById('check1');
          var b=document.getElementById('check2');
          var c=document.getElementById('check3');
          var d=document.getElementById('check4');
          var e=document.getElementById('check5');
          var f=document.getElementById('check6');
          var g=document.getElementById('check7');
          ...
          if(a.checked)
          {
            for(const [key, i] of Object.entries(hash[a.value]) )
            {
              ans.push(i);
            }
          }
          if(b.checked)
          {
            for(const [key, i] of Object.entries(hash[b.value]) )
            {
              ans.push(i);
            }
          }
          if(c.checked)
          {
            for(const [key, i] of Object.entries(hash[c.value]) )
            {
              ans.push(i);
            }
          }
          if(d.checked)
          {
            for(const [key, i] of Object.entries(hash[d.value]) )
            {
              ans.push(i);
            }
          }
          if(e.checked)
          {
            for(const [key, i] of Object.entries(hash[e.value]) )
            {
              ans.push(i);
            }
          }
          if(f.checked)
          {
            for(const [key, i] of Object.entries(hash[f.value]) )
            {
              ans.push(i);
            }
          }
          if(g.checked)
          {
            for(const [key, i] of Object.entries(hash[g.value]) )
            {
              ans.push(i);
            }
          }
          
          b=document.getElementById('Attributes').value;
          var maxv=0;
          var minv=1000000;

          data1.forEach(function (arrayItem) {
                
                var x = arrayItem.Country;
                if(ans.indexOf(x)!=-1){
                      var st=arrayItem.Country;
                      maxv=Math.max(maxv,arrayItem[b]);
                      minv=Math.min(minv,arrayItem[b]);
                      if(arrayItem.Year>=year0 && arrayItem.Year<=year1){
                      if(arrayItem.Year<=slider){
                      d1.push({'Year':new Date(arrayItem.Year),'Country':arrayItem.Country,'Region':arrayItem["World bank region"],'Value':+arrayItem[b]});
                                                }}
              }
            });

    char_data = d3.group(d1, d => d.Country);
    char_data = Array.from(char_data, ([key, value]) => ({ key, value }));
    

x.domain([new Date(year0),new Date(year1)])
...
updateChart(char_data,opacity);

}
function makeLines(idList) {
  return idList.map((id, index) => `line${index+1}`);
}


function updateChart(char_data,opacity) {
 
    var id_list = makeLines(char_data);
  ...
  .range(["#a1b8c9", "#e8ae8d" , "#D62728","#9467BD","#779bda","#E377C2","#2CA02C",]);

  var line = d3.line().x(function(d) { return x(d["Year"]);}).y(function(d) { return y(d["Value"]); });

   var lines = svg.selectAll(".line").data(char_data)
  
  lines.enter().append("path").merge(lines).attr("class", "line").attr("fill", "none")
        .style("stroke", function(d){ return color(d.value[0]["Region"]) }).attr("stroke-width", 2).attr("pointer-events","visible")
        .attr("id", function(d,i){
          return id_list[i%id_list.length]})
        .attr("d", function(d){
          return line(d.value)
        }).style("opacity", 0);

        lines.on('mouseover', function(d, i) {
          const hoveredId = d.originalTarget.id;
          id_list.forEach((id) => {
          ...
          id_list.forEach((id) => {
              const lineId = `line${id}`;
              d3.select(`#${lineId}`).style("opacity", opacity);
          });
      });
  ...
  lines.enter().append("path").merge(lines).attr("class", "line").attr("fill", "none")
        ...
        function highlightLines(d, i) {
          for (var index = 0; index < id_list.length; index++) {
            var a2 = 'line' + (index + 1);
            if (a2 == d.originalTarget.id) {
              d3.selectAll("#" + a2).style("opacity", 1);
            } else {
              d3.selectAll("#" + a2).style("opacity", 0.1);
            }
          }
        }
        
        function restoreLines(d, i) {
          for (var index = 0; index < id_list.length; index++) {
            var a2 = 'line' + (index + 1);
            d3.selectAll("#" + a2).style("opacity", opacity);
          }
        }
        
        lines.on('mouseover', highlightLines).on('mouseout', restoreLines);
        

   var circles = svg.selectAll(".circle").data(char_data);
  circles.exit().remove()
  circles.enter().append('circle').merge(circles)
      //.transition().duration(1500)
      .attr("class","circle").attr('cx', function(d){
            return x(d3.max(d.value, function(i) { 
              var d2=new Date(i["Year"]);
              d2.setMonth(d2.getMonth()+1);
              return d2; }) );
          })
          .attr('cy', function(d){
            maxyear = d3.max(d.value, function(i) { return i["Year"]; })
            for(var i=0;i<d.value.length;i++){
              if(d.value[i]["Year"] == maxyear)
              return y(d.value[i]['Value']);
            }
          })
          .style('fill', function(d){return color(d.value[0]["Region"])})
          .attr("id", function(d,i){return id_list[i%id_list.length]})
          .attr("pointer-events","visible").style('opacity',opacity).attr('r',5);  
         ...
         .on("mouseout", function(d) {
          id_list.forEach(function(id){
               d3.selectAll("#"+id).style("opacity", opacity);
             });
         });
        ...
        var texts = svg.selectAll(".text")
          ...
          //.transition().duration(1500)
          .attr('x',function(d){
             return 10+x(d3.max(d.value, function(i) { return i["Year"]; }) );
           })
          .attr('y',function(d){
            maxyear = d3.max(d.value, function(i) { return i["Year"]; })
            for(var i=0;i<d.value.length;i++){
          ...
          .attr('fill', function(d){return color(d.value[0]["Region"])})
          .attr("id", function(d,i){return id_list[i%id_list.length]})
    ...
    texts.on('mouseover', function(d, i){
     id_list.forEach((_, index) => {
      const lineId = `line${index + 1}`;
      const isSelected = lineId === d.originalTarget.id;
    
      d3.selectAll(`#${lineId}`).style("opacity", isSelected ? 1 : 0.1);
    });
  ...
  .on("mouseout", function(d) {
    id_list.forEach((_, index) => {
      const lineId = `line${index + 1}`;
      d3.selectAll(`#${lineId}`).style("opacity", opacity);
    });
...
var imgs = svg.selectAll(".image").data(char_data);
    imgs.exit().remove()
    imgs.enter().append('svg:image').attr("class", "image").merge(imgs)
    //.transition().duration(1500)
    .attr('x', d => 10 + x(d3.max(d.value, i => i.Year)))
.attr('y', d => {
  const maxYear = d3.max(d.value, i => i.Year);
  const value = d.value.find(i => i.Year === maxYear)?.Value ?? 0;
  return y(value + 1);
})
    .attr("width", "20").attr("height", "20").attr("id", function(d,i){return id_list[i%id_list.length]}).attr("pointer-events","visible")
    
    .attr("xlink:href", function(d){
      if(d.key in CountryFlags)
      {
        return CountryFlags[d.key]["normal"]
      }
      else{
        return "https://upload.wikimedia.org/wikipedia/commons/thumb/4/49/A_black_image.jpg/1200px-A_black_image.jpg?20201103073518"
      }
    }).attr('fill-opacity',opacity);    

    imgs.on('mouseover', function(d, i){
      id_list.forEach((_, index) => {
        const lineId = `line${index + 1}`;
        const opacity = lineId === d.originalTarget.id ? 1 : 0.1;
        d3.selectAll(`#${lineId}`).style("opacity", opacity);
      });
